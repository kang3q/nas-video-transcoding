...
**이 전제는 검증된 것이 아니라 추정입니다. 본인 라이브러리와 플레이어로 직접
확인하세요.** 특히 `hevc` 를 재생 가능 목록에 넣어둔 것은 Apple TV 가 하드웨어로
디코딩한다는 가정에 기댄 것이며, 기기 세대에 따라 성립하지 않습니다.

검증에는 **실제로 쓸 플레이어와 티어**를 써야 합니다. 상위 플레이어나 유료
버전은 대부분의 코덱을 재생하므로, 무엇을 변환해야 하는지 알려주지 못합니다. Firecore의 공개 문서는 무료와
...
문제는 **그 첫 요청이 재생을 시작하는 요청과 완전히 같다**는 것입니다. nPlayer
로 관측한 실제 순서입니다 (플레이어마다 세부는 다르지만, 첫 요청이 구분되지
않는다는 점은 같습니다):
...
요청 하나만 봐서는 구분할 수 없으므로, 모양이 아니라 **폭**으로 판정합니다.
폭은 플레이어에 의존하지 않습니다 — 라이브러리를 만드는 쪽은 폴더를 훑고,
보는 쪽은 파일 하나만 건드립니다. 훑기는 몇 초 안에
...
기본값(10초/2개)은 nPlayer 로 관측한 훑기 속도에 맞춰 잡았습니다. 다른
플레이어에서는 조정이 필요할 수 있습니다.
